...
class DirZCtl:
    ...
    def _build_config(self):
        ...
        for i in range(len(self.steppers)):
            dir_pin, step_pin, ivt_dir, ivt_step = self.steppers[
                i
            ].get_pin_info()
            self.mcu.add_config_cmd(
                "add_dirzctl oid=%d index=%d dir_pin=%s step_pin=%s dir_invert=%d step_invert=%d"
    ...
    def get_params(self):
        return self.all_params, (
            self.all_params[0]["tick"] if len(self.all_params) > 0 else 0
        )

    def check_and_run(
        self, direct, step_us, step_cnt, wait_finish=True, is_ck_con=False
    ):
        ...
        while (
            ...
            and (
                (time.time() - t_start)
                < (1.5 * 1000 * 1000 * step_us * step_cnt)
            )
            and len(self.all_params) != 2
        ):
    ...
    def cmd_DIRZCTL(self, gcmd):
        index = gcmd.get_int(
            "INDEX", len(self.steppers), minval=0, maxval=len(self.steppers)
        )
