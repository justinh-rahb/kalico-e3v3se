...
class RCHFilter:
    ...
    def ftr_val(self, vals):
        out_vals = [0]
        rc = 1.0 / 2.0 / math.pi / self.cut_frq_hz
...
class Filter:
    def __init__(self, config):
        self.hft_hz = config.getfloat(
            "hft_hz", default=5, minval=0.1, maxval=10.0
        )
        self.lft_k1 = config.getfloat(
            "lft_k1", default=0.8, minval=0.0, maxval=1.0
        )
        self.lft_k1_oft = config.getfloat(
            "lft_k1_oft", default=0.8, minval=0.0, maxval=1.0
        )
        self.lft_k1_cal = config.getfloat(
            "lft_k1_cal", default=0.8, minval=0.0, maxval=1.0
        )
        pass
